...
//    val spark = SparkSession.builder().master("local").getOrCreate()
    ...
    summaryDF = summaryDF.withColumn("birthMonth", toDouble(summaryDF("birthMonth")))
...
//    )
    ...
    val catFeatureOneHotEncoders = categoricalColumns
      .map(x => new OneHotEncoder().setInputCol(x).setOutputCol(s"${x}_oh"))
...
//    ).union(catFeatureOneHotEncoders.map(_.getOutputCol))
    ...
    val Array(trainingData, testData) = summaryDF.randomSplit(Array(0.7, 0.3))

    val pipeline = new Pipeline().setStages(catFeatureOneHotEncoders.union(Array(featureAssembler, labelIndexer, lr))).fit(trainingData)
    ...
    saveToModelFile(summaryDF, pipeline, "/Users/eliu/Desktop/claims_prediction_2features.txt")
